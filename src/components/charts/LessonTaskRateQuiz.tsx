...
const LessonTaskRateQuiz = ({
  ...
  const quizDataKeys = Object.keys(data.QUIZ ?? {});

  const parsedQuizData = quizDataKeys.map((item, idx) => ({
    id: item.toLowerCase(),
    label: item[0] + item.slice(1).toLowerCase(),
    value: data.QUIZ[item],
    color: idx === 0 ? 'tomato' : idx === 1 ? 'goldenrod' : 'lightblue',
  }));

  const totalquizSum = quizDataKeys.reduce((acc, curr) => {
    return acc + data.QUIZ[curr];
  }, 0);
            ...
            >
              {totalquizSum > 0 ? (
                ...
                <div className='mt-32 lg:mt-10'>
                  <EmptyView label='No Quiz Submission' />
                </div>
              )}
            </div>
          </div>
...
export default LessonTaskRateQuiz;
