...
const LessonTaskRateAssignment = ({
  data,
  institute,
  showLink = false,
}: AttendanceRateProps) => {
  ...
  const assignmentDataKeys = Object.keys(data.ASSIGNMENT ?? {});

  const parsedAssignmentData = assignmentDataKeys.map((item, idx) => ({
    id: item.toLowerCase(),
    label: item[0] + item.slice(1).toLowerCase(),
    value: data.ASSIGNMENT[item],
    color: idx === 0 ? 'tomato' : idx === 1 ? 'goldenrod' : 'lightblue',
  }));

  const totalAssignmentSum = assignmentDataKeys.reduce((acc, curr) => {
    return acc + data.ASSIGNMENT[curr];
  }, 0);

  const isEmpty = !data || !assignmentDataKeys?.length;
            ...
            >
              {totalAssignmentSum > 0 ? (
                ...
                <div className='mt-32 lg:mt-10'>
                  <EmptyView label='No Assignment Submission' />
                </div>
              )}
            </div>
          </div>
...
export default LessonTaskRateAssignment;
