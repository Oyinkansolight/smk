...
const AddClass = () => {
  ...
  const { data: allPermission, isLoading: isLoadingPermissions } =
    useGetPermissionList(pagingData);
  ...
  const handleTransferToActivated = (e) => {
    ...
    const uncheckAllSelected = selected.map((v: any) => ({
      ...v,
      checked: false,
    }));
  ...
  const handleTransferToPermissions = (e) => {
    e.preventDefault();
    const selected = activatedPermissions.filter(
    ...
    setActivatedPermissions(
      activatedPermissions.filter((v: any) => v.checked === false)
    );
  };
  ...
  return (
    <section className='md:px-[60px] px-5 py-6'>
      ...
      <form
        ...
        className='table-add-permission mt-7 lg:px-20 px-4 py-10 pb-4 bg-white'
      >
          ...
          <div className='grid grid-cols-12 gap-2 items-center justify-center'>
            <div className='col-span-5'>
              ...
              <div className='flex flex-row justify-center gap-4 mt-4'>
                ...
                <Button
                  disabled={
                    allPermission?.paging?.totalPage === pagingData.page
                  }
                  onClick={handleNextPage}
                >
                  Next
                  <IoIosArrowForward className='text-xl font-normal' />{' '}
                </Button>
              </div>
            </div>
            ...
            <div className='col-span-2'>
              <div className='space-y-16 flex flex-col items-center justify-center'>
                <button
                  onClick={handleTransferToActivated}
                  className='bg-[#F6F9FC] p-2 rounded-md w-min'
                >
                  <IoIosArrowForward className='text-[#008146] text-4xl font-normal' />
                </button>
                <button
                  onClick={handleTransferToPermissions}
                  className='bg-[#F6F9FC] p-2 rounded-md w-min'
                >
            ...
            <div className='col-span-5'>
              ...
              <div className='border bg-[#FFF] p-2 rounded-lg w-full h-[450px] overflow-hidden overflow-y-scroll'>
                ...
                {activatedPermissions.map((v, id) => (
                  <div key={id} className='flex space-x-3 p-3 items-center'>
                    <input
                      onChange={() => handleActivatedCheck(v.id)}
                      checked={v.checked}
                      type='checkbox'
                    />
        ...
        <div className='flex justify-end'>
          <button
            type='submit'
            className='border  bg-[#016938] text-white rounded px-4 py-2'
          >
            Create
          </button>
        </div>
