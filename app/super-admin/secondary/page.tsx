...
const AddStudent = () => {
  ...
  const [secondaryschools, setsecondaryschools] = useState(mockData);

  const handleSearch = (value: string) => {
    const result = mockData.filter((data) =>
      data.name.toLowerCase().includes(value.toLowerCase())
    );
    setsecondaryschools(result);
  };
  ...
  return (
    <section className='px-[60px] py-6'>
      ...
      <h1 className='mt-5 mb-6 text-2xl font-bold'>Secondary Schools</h1>

      <div className='mb-6 flex justify-between items-end'>
        <div className='bg-[#FFF6EC] p-3 rounded-2xl w-[200px]'>
          <p className='text-[#615F5F]'>Total Secondary Schools</p>
