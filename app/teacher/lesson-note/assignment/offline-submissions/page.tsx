...
export default function Page() {
  return (
    <div className='layout w-full flex flex-col gap-4'>
      ...
      <div className='flex font-bold text-[#746D69]'>
...
function GradeListItem({
  ...
  return (
    <div className='flex gap-6'>
      <div className='flex-1 px-5 text-center items-center border rounded-md flex h-12'>
        <div>{index + 1}</div>
        <div className='w-10' />
        <div className='h-10 w-10 bg-slate-200 rounded-full px-4' />
        <div className='text-bold'>{studentName}</div>
      </div>
      <div className='w-56 text-center rounded-md border h-12 flex items-center justify-center'>
        <div>{score}</div>
      </div>
    </div>
  );
